...
class ObjectUtils {
    ...
    static getPropertyValueByNamePath(sourceObject, namePath) {
        let names = ObjectUtils.splitNamePath(namePath); // 不能简单地使用 String.split('.') 函数分割
     ...
     */
    static composeObject(sourceObject, propertyNames) {
        let targetObject = {};
     ...
     * @returns 返回只由指定属性所构成的新对象
     */
    static composeObjectByProperityNameSequence(sourceObject, nameString) {
        let propertyNames = ObjectUtils.splitProperityNameSequence(nameString);
        return ObjectUtils.composeObject(sourceObject, propertyNames);
    }
